...
if (_LiquidGlassFlipBlurTexX)
	grabUV.x = 1.0 - grabUV.x;
if (_LiquidGlassFlipBlurTexY)
grabUV.y = 1.0 - grabUV.y;

float2 distortionCoord = grabUV + refractVec.xy * refractLength / 100;
half4 refractColor = tex2D(_GrabTexture, distortionCoord.xy);
float3 reflectVec = reflect(incident, normal);
half4 reflectColor = float4(0.0, 0.0, 0.0, 0.0);

float c = clamp(abs(reflectVec.x - reflectVec.y), 0.0, 1.0);
reflectColor = float4(c, c, c, 0.0);

half4 bg = lerp(refractColor, reflectColor, (1.0 - normal.z) * 2.0 * (_LiquidGlassEdgeReflect - 1.0));
bg = clamp(bg, 0., 1.);
...
#endif
	half4 layer0 = half4(lerp(outlineColor.rgb, _OutlineEffectColor.rgb, outlineEffect * _OutlineEffectColor.a), outlineColor.a);
#else
...
#endif
	half4 layer1 = half4(lerp(graphicColor.rgb, _GraphicEffectColor.rgb, graphicEffect * _GraphicEffectColor.a), graphicColor.a);
#else
	half4 layer1 = graphicColor;
#endif

	half4 layer2 = lerp(layer0, layer1, graphicAlpha);
	layer2.a *= outlineAlpha;
	effects = layer2;
}
